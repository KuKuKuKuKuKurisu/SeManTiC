...
class Dataset(data.Dataset):
    ...
    # Constants.
    EMPTY_IMAGE = torch.zeros(3, 224,
                              224)
    EMPTY_PRODUCT_TEXT = ''

    def __init__(self, task, image_paths, dialogs, preprocess):
        self.task = task
    ...
    def _get_context_dialog(self, dialog):
        ...
        for utter_id, utter in enumerate(dialog[:-1]):
            ...
            if text_speaker == 'user':
                text_speaker = clip.tokenize('user', context_length = 1, truncate = True) * DatasetConfig.dialog_text_max_len
            elif text_speaker == 'system':
                text_speaker = clip.tokenize('system', context_length = 1, truncate = True) * DatasetConfig.dialog_text_max_len
        ...
        text_type.append(clip.tokenize('query', context_length = 1, truncate = True) * DatasetConfig.dialog_text_max_len)
        turn_mask.append(1)
    ...
    def _get_images_product_texts(self, image_ids, image_num, image_weight, max_image_num):
        ...
        product_temp_text = clip.tokenize(product_texts, context_length = DatasetConfig.clip_max_len, truncate = True)
        product_texts = {}
        product_texts['input_ids'] = product_temp_text
        product_texts['attention_mask'] = product_temp_text.gt(0)
        product_texts['text_type'] = torch.tensor([clip.tokenize('item', context_length = 1, truncate = True) * DatasetConfig.product_text_max_len] * len(images))
