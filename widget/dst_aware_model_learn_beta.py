...
class DST_AWARE(nn.Module):
    ...
    def get_state_emb(self, dst_ids, dst_attention_mask, batch_size, return_key_only = False):
        ...
        return state_key_emb, state_value_emb
    ...
    def turn_level_self_attn(self, context_txt_emb, context_img_emb, turn_mask, image_mask, state_key_embedding, state_embedding):
        ...
        pred_state_embedding, = self.state_value_pred(context_emb, state_key_embedding, non_pad_mask, attn_mask)
    ...
    def forward(self, context_dialog, pos_products, neg_products, eval = False):
        ...
        pred_state_emb = self.turn_level_self_attn(torch.cat(context_text_embeddings, dim = 1), torch.cat(context_image_embeddings, dim = 1), turn_mask, image_mask,
        ...
        if state_embedding!=None:
            # kl div
            pred_context_txt_emb, pred_context_text_kl = pred_context_txt_emb
            pred_context_img_emb, pred_context_image_kl = pred_context_img_emb
            context_txt_emb, context_text_kl = context_txt_emb
            context_img_emb, context_image_kl = context_img_emb

            hist_kl_learn_loss = (self.cal_kl_loss(torch.log(pred_context_text_kl), context_text_kl) +
                                  self.cal_kl_loss(torch.log(pred_context_image_kl), context_image_kl))
        ...
        for neg_id in range(DatasetConfig.max_neg_num):
            ...
            if eval == False:
                if state_embedding!=None:
                    ...
                    # kl learn loss
                    prod_kl_learn_loss.append(self.cal_kl_loss(torch.log(neg_pred_text_embedding[1]), neg_text_embedding[1]) +
                                              self.cal_kl_loss(torch.log(neg_pred_image_embedding[1]), neg_image_embedding[1]))
                else:
                    # neg text image sim
                    prod_txt_embeddings.append(neg_pred_text_embedding[0])
                    prod_img_embeddings.append(neg_pred_image_embedding[0])
                    # neg kl
        ...
        for pos_id in range(DatasetConfig.max_pos_num):
            ...
            if eval == False:
                if state_embedding!=None:
                    ...
                    # kl learn loss
                    prod_kl_learn_loss.append(self.cal_kl_loss(torch.log(pos_pred_text_embedding[1]), pos_text_embedding[1]) +
                                              self.cal_kl_loss(torch.log(pos_pred_image_embedding[1]), pos_image_embedding[1]))
                else:
                    # pos text image sim
                    prod_txt_embeddings.append(pos_pred_text_embedding[0])
                    prod_img_embeddings.append(pos_pred_image_embedding[0])
                    # pos kl
        ...
        if eval:
            ...
            if (state_embedding!=None) & (RecommendTrainConfig.start_learn):
                ...
                # pred_pos_mask = get_mask(DatasetConfig.max_pos_num, pos_imgs_num, GlobalConfig.device)
                # pred_mask = torch.cat([pred_neg_mask, pred_pos_mask], dim=-1)
                # prod_kl_learn_loss = torch.stack(prod_kl_learn_loss, dim = 1).masked_select(pred_mask.bool())
                # kl_learn_loss = torch.cat([hist_kl_learn_loss, prod_kl_learn_loss]).mean()
                kl_learn_loss = 0

                # total loss
                student_loss = pred_loss + learn_loss
                loss = (1 - RecommendTrainConfig.learn_loss_weight) * loss + RecommendTrainConfig.learn_loss_weight * student_loss
            else:
                learn_loss = 0
                pred_loss = 0
                kl_learn_loss = 0

            return loss, (sim_loss, txt_img_loss, kl_loss, pred_loss, learn_loss, kl_learn_loss)